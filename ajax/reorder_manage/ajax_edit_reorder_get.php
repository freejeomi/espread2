...
//    GET THE STOCK
    ...
    $sql_stock_dis="SELECT stock_id,stock_code,stock_name,reorder_level FROM stock ORDER BY $sidx $sord ";
...
//            THERE ARE STORES
            if($number_store>0){
                ...
                while($row = mysqli_fetch_assoc($result_dis)) {
                    $array_reorder[$i]=[$serial,$row['stock_id'],$row['stock_name'],$row['stock_code'], $row['reorder_level']];
                    $stockid=$row['stock_id'];
                foreach($store_id as $storeid){
                    $result_store_dis=mysqli_query($conn,"SELECT reorder_level_store FROM stockposition WHERE stock_id=$stockid and store_id=$storeid");
            ...
            //NO STORES, JUST ECHO STOCK AND GENERAL
               ...
               //FETCH THE DATA AND PLACE IT IN RESPONCE
                while($row = mysqli_fetch_assoc($result_dis)) {
                    ...
                    $responce->rows[$i]['id']=$row['stock_id'];
                    $responce->rows[$i]['cell']=array($serial,$row['stock_id'],$row['stock_name'],$row['stock_code'], $row['reorder_level']);
