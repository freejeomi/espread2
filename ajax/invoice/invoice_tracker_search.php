...
//to assign cashier
if(isset($_POST['cashier_selected']) && isset($_POST['invoice_num_obtained'])){
    if($_POST['cashier_selected']=='select_cashier'){
        $data['message']="Please select a cashier";
        $data['success']=false;
    }
    else{
        ...
        if(mysqli_num_rows($result) == 0){
            mysqli_query($conn, "UPDATE salesinvoice SET cashier='$cashier_select' where invoice_num='$invoice_num_obtained'");
            $data['message'] = "Invoice Num <b>" . $invoice_num_obtained . " </b>has been assigned to <b>" . $cashier_select . "</b>.";
            $data['success'] = true;
        }
        else {
            mysqli_query($conn, "UPDATE salesinvoice_daily SET cashier='$cashier_select' where invoice_num='$invoice_num_obtained'");
            $data['message'] = "Invoice Num <b>" . $invoice_num_obtained . " </b>has been assigned to <b>" . $cashier_select . "</b>.";
            $data['success'] = true;
        }
...
elseif(isset($_POST['invoice_num_hidden'])){
    ...
    $result= mysqli_query($conn, "SELECT * FROM salesinvoice_daily where invoice_num='$invoice_num_hidden'");
    if (mysqli_num_rows($result) > 0) {
        mysqli_query($conn, "UPDATE salesinvoice_daily SET cashier='' where invoice_num='$invoice_num_hidden'");
        $data['message'] = "Invoice Num <b>" . $invoice_num_hidden . " </b> has been unassigned it's cashier.";
        $data['success'] = true;
    }
    else {
        mysqli_query($conn, "UPDATE salesinvoice SET cashier='' where invoice_num='$invoice_num_hidden'");
        $data['message'] = "Invoice Num <b>" . $invoice_num_hidden . " </b> has been unassigned it's cashier.";
        $data['success'] = true;
    }
}

//to unassign cashier from the daily sales ledger grid
elseif (isset($_POST['invoice_num_grid']) ) {
    $invoice_num_grid = $_POST['invoice_num_grid'];
    $result_cash= mysqli_query($conn, "UPDATE salesinvoice_daily SET cashier='' where invoice_num='$invoice_num_grid'");
    if(! $result_cash) {
        $data['error'] = "An error occurred while trying to update cashier. please try again";
        $data['success'] = false;
    }
    else{
    $data['message'] = "Invoice Num <b>" . $invoice_num_grid . " </b> has been unassigned its cashier.";
    $data['success'] = true;
    }
...
//to display table
    ...
    if (empty($invoice_num)) {
        $data['message'] = "Please enter the invoice number";
        $data['success'] = false;
    } //there is an invoice number/// check database oooo
    else {
        $sql_salesinvoice = "SELECT invoice_num,customer_name,cashier FROM salesinvoice_daily INNER JOIN customer ON customer.customer_id = salesinvoice_daily.customer_id WHERE invoice_num='$invoice_num'";
        $result_invoice = mysqli_query($conn, $sql_salesinvoice);

        //check if the invoice exists
        if (mysqli_num_rows($result_invoice) == 0) {
        //check if it exists in sales invoice table
            $sql_salesinvoice = "SELECT invoice_num,customer_name,cashier FROM salesinvoice INNER JOIN customer ON customer.customer_id = salesinvoice.customer_id WHERE invoice_num='$invoice_num'";
            $result_invoice = mysqli_query($conn, $sql_salesinvoice);
            ...
            while ($row_invoice = mysqli_fetch_assoc($result_invoice)) {
                ...
                if ($invoice_cashier != null) {
                    ...
                    if (mysqli_num_rows($result_invoiceitem) > 0) {
                        //put them in a table
                        while ($row_invoiceitem = mysqli_fetch_assoc($result_invoiceitem)) {
                            $items_row .= '<tr><td>'.$s.'</td><td>' . $row_invoiceitem['stock_name'] . '</td><td>' . number_format( $row_invoiceitem['quantity']) . '</td><td>' .  number_format(['charge'],2) . '</td></tr>';
                            $s++;
                            ...
                            if (mysqli_num_rows($sql_promo) > 0) {
                                while ($row_promo = mysqli_fetch_assoc($sql_promo)) {
                                    $promo_table .= '<tr><td>'.$p.'</td><td>' . $row_promo['stock_name'] . '</td><td>' .  number_format(['quantity']) . '</td><td>0.00</td></tr>';
                                    $p++;
                                }
                            }
                            //invoice exists so search the items daily table
                        ...
                        //put them in a table
                        while ($row_invoicesum = mysqli_fetch_assoc($result_invoicesum)) {
                            $items_row .= $promoheader . $promo_table . '<tr class="bg-success"><td colspan="3" class="text-center"><b>TOTAL</b></td><td><b>' .  number_format(['total_amount']) . '</b></td></tr>';
                        }
                        $items_table .= $items_row . '</tbody></table></div>';
                        //display total in a table
//                    $items_total .= '
